...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    ...
    // دورة ذكية: تظهر الرسالة لمدة 4 ثوانٍ ثم تختفي لتترك مساحة بصرية نظيفة (تتكرر الدورة)
    const interval = setInterval(() => {
      setShowMessage(true);
      setTimeout(() => setShowMessage(false), 4000);
    }, 9000); // تتكرر كل 9 ثوان
  ...
  return (
    // تم وضع pointer-events-none للحاوية حتى لا تمنع الضغط على أي شيء خلفها
    <div className="fixed bottom-33 md:bottom-9 right-6 z-[99] flex flex-col items-end gap-3 pointer-events-none">
      ...
      <AnimatePresence>
        {showMessage && (
          <motion.div
            ...
            className="relative bg-[#1a1a1a] text-[#D4AF37] border border-[#D4AF37]/20 shadow-[0_4px_15px_rgba(0,0,0,0.3)] rounded-xl px-5 py-2.5 text-sm font-medium tracking-wide ar pointer-events-auto cursor-default"
          >
            تواصل معنا الآن
            {/* المثلث الصغير الموجه للأسفل */}
            <div className="absolute -bottom-1.5 right-[22px] w-3 h-3 bg-[#1a1a1a] border-b border-r border-[#D4AF37]/20 transform rotate-45" />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <a
        ...
        onMouseEnter={() => setShowMessage(true)} // إظهار الرسالة عند وقوف الماوس
        onMouseLeave={() => setShowMessage(false)} // إخفاء الرسالة عند إبعاد الماوس
