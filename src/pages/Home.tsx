...
import { Phone, Instagram, MapPin, Home as HomeIcon, Layers, Grid3x3, Sun, Lamp, Map, Star, Wrench, PaintBucket, Box, Square, X } from "lucide-react";
import Nav from "@/components/Nav";
import BrandLogo from "@/components/BrandLogo";

/* ─── المكونات الجمالية والأدوات ─── */
...
/* ─── المعرض اللانهائي (Infinite Seamless Loop) ─── */
const InfiniteMarquee = ({ images, duration, reverse = false, onImageClick }: any) => {
  const duplicatedImages = [...images, ...images, ...images];
  return (
    <div className="relative w-full overflow-hidden py-10" dir="ltr"> {/* فرض الاتجاه هنا يحمي الأنيميشن من التداخل */}
      ...
      >
        {duplicatedImages.map((img, i) => (
          <div 
            ...
            className="relative w-[300px] md:w-[400px] aspect-[4/5] shrink-0 cursor-zoom-in overflow-hidden border border-gold/10 group hover:border-gold/50 transition-colors"
          >
...
export default function HomePage() {
  ...
  return (
    <div className="bg-charcoal min-h-screen text-white overflow-x-hidden">
      ...
      <AnimatePresence>
        {selectedImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedImage(null)} className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/95 p-6 backdrop-blur-md cursor-zoom-out">
             <motion.img initial={{ scale: 0.9 }} animate={{ scale: 1 }} src={selectedImage.src} className="max-w-full max-h-[85vh] object-contain shadow-2xl" />
             <button onClick={() => setSelectedImage(null)} className="absolute top-10 right-10 text-white/50 hover:text-gold transition"><X size={40}/></button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <section id="portfolio" className="py-24 bg-[#0a0907]">
        <div className="container mx-auto px-6 mb-12">
            <Reveal><h2 className="text-3xl font-serif">معرض الإبداع</h2></Reveal>
        </div>
        ...
        <InfiniteMarquee 
          ...
          onImageClick={setSelectedImage}
          images={[
            { src: "/work-4.jpeg", label: "الممرات" },
            { src: "/work-5.jpeg", label: "جلسات خارجية" },
            { src: "/work-6.jpeg", label: "منطقة الطعام" },
            { src: "/marble-4.jpeg", label: "بورسلين" },
         ...
         <div className="grid md:grid-cols-2 gap-12 items-center">
             <Reveal>
                ...
                <div className="space-y-8">
                   <a href="https://wa.link/gitycp" className="flex items-center gap-6 text-xl hover:text-gold transition-colors"><Phone className="text-gold" /> +968 7753 3603</a>
                   <a href="#" className="flex items-center gap-6 text-xl hover:text-gold transition-colors"><MapPin className="text-gold" /> سلطنة عُمان</a>
                </div>
             </Reveal>
         </div>
      </section>
      
      <footer className="py-10 text-center text-white/30 text-xs border-t border-white/5">© 2026 Future Design Decore</footer>
    </div>
  );
}
