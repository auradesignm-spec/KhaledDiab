...
{/* ══════════════════════════════════════════
          1. HERO (وميض احترافي + توسط مطلق للموبايل واللابتوب)
      ══════════════════════════════════════════ */}
      <section className="relative h-[100dvh] w-full bg-charcoal flex flex-col items-center justify-center overflow-hidden">
        ...
        {/* الحاوية المركزية التي تضمن التوسط دائماً */}
        <div className="relative z-10 text-center max-w-4xl px-6 flex flex-col items-center w-full">
          
          <motion.div initial={{ scale: 0.88, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 1.2, ease: [0.16, 1, 0.3, 1] }} className="mb-4">
            {/* 🌟 الوميض الذهبي الآمن للصور المفرغة */}
            <motion.div
              animate={{ y: [0, -6, 0] }}
              transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
              className="relative inline-block"
            >
              <BrandLogo className="w-20 h-14 md:w-24 md:h-16 lg:w-28 lg:h-20 text-cream-light" />
              
              {/* لمعة ذهبية تمر فوق الشعار المفرغ فقط */}
              <motion.div
                animate={{ x: ["-100%", "200%"] }}
                transition={{ duration: 3, repeat: Infinity, ease: "linear", repeatDelay: 3 }}
                className="absolute inset-0 z-10 w-full h-full bg-gradient-to-r from-transparent via-white/40 to-transparent skew-x-[-20deg]"
                style={{ maskImage: "url(/logo-path-or-similar)", WebkitMaskImage: "inherit" }} 
              />
            </motion.div>
          </motion.div>
          
          <motion.div initial={{ scaleY: 0 }} animate={{ scaleY: 1 }} transition={{ duration: 0.8, delay: 0.3, ease: "easeOut" }} className="w-[1px] h-10 bg-gradient-to-b from-transparent via-gold to-transparent mx-auto mb-4" />
          
          <motion.p initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 0.4 }} className="font-sans font-medium text-[10px] uppercase mb-2 text-gold-light tracking-[0.2em]">
            <span className="ar">خالد دياب</span> <span className="en">Khaled Diab</span>
          </motion.p>
          
          <motion.h1 initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1, delay: 0.5 }} className="font-serif text-5xl md:text-7xl text-cream-light leading-none mb-1">
            Future Design
          </motion.h1>
          <motion.h1 initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1, delay: 0.6 }} className="font-serif italic text-5xl md:text-7xl text-gold leading-none mb-6">
            Decore
          </motion.h1>
          
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="max-w-xl mx-auto mb-8">
            <p className="font-sans font-light text-xs text-cream/60 uppercase tracking-widest">
              <span className="ar block md:inline">تصميم داخلي · تنفيذ احترافي</span>
            </p>
          </motion.div>
          
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1 }}>
            <a href="/#design-services" className="relative px-8 py-3 border border-gold/40 text-gold-light font-sans text-[10px] uppercase tracking-[0.2em] hover:bg-gold hover:text-charcoal transition-all">
